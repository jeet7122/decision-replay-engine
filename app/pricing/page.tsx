        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-start p-8 bg-/20">
            {PLANS.map((plan) => (
                <div key={plan.name} className="border p-6 rounded-xl shadow-lg w-80">
                    ...
                    <p className="text-gray-600 mb-4">${plan.price}/{plan.interval}</p>
                    ...
                    {plan.planId !== "free" ? (
                        <button
                            onClick={()=>handleCheckout(plan.planId as "pro" | "pro_plus")}
                            className="w-full bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700"
                        >
                            Upgrade
                        </button>
                    ) : (
                        <button disabled className="w-full bg-gray-300 text-gray-700 py-2 px-4 rounded cursor-not-allowed">
                            Free
                        </button>
                    )}
                </div>
